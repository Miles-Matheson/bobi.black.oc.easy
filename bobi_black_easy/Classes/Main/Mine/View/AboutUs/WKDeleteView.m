...
- (UITextField *)textEmail {
    if (!_textEmail) {
        ...
        _textEmail.placeholder = @"please input your passWord";
...
- (UILabel *)lblNote {
    if (!_lblNote) {
        ...
        _lblNote.text = @"If you delete your account, you will permanently lose\nyour profile, messages, photo.";
...
- (UIButton *)btnContinue {
    if (!_btnContinue) {
        ...
        [_btnContinue setTitle:@"Submit" forState:UIControlStateNormal];
...
- (void)onbtnContinueClick {
    ...
    if (![self.textEmail.text isEqualToString:[[WKUserInforManager sharedInstance] getUserInforPassword]]) {
        [YPCSVProgress showStatusMessageWithTitle:@"Please enter the correct password" finishBlock:nil];
        return;
    }
    [self routerEventWithName:@"deleteAcountClickAction" userInfo:@{}];
}
