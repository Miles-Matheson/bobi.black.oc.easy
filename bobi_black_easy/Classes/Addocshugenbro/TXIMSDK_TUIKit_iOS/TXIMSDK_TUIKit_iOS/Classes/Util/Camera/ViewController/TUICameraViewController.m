...
@interface TUICameraViewController ()<AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate, TUICameraViewDelegate,TZImagePickerControllerDelegate>
{
    // 会话
...
-(void)stopRecordVideoAction:(TUICameraView *)cameraView RecordDuration:(CGFloat)duration {
    ...
    [_movieManager stop:^(NSURL * _Nonnull url, NSError * _Nonnull error) {
        if (duration < self.videoMinimumDuration) {
            [self showErrorStr:TUILocalizableString(TUIKitMoreVideoCaptureDurationTip)];
        }
        else if (error) {
            [self showErrorStr:error.localizedDescription];
        }
...
- (void)showErrorStr:(NSString *)errStr {
    [THelper makeToast:errStr duration:1 position:CGPointMake(self.view.bounds.size.width / 2.0, self.view.bounds.size.height / 2.0)];
}
