...
- (void)fillWithData:(TCommonContactSelectCellData *)selectData
{
    ...
    if (selectData.avatarUrl) {
        [self.avatarView sd_setImageWithURL:selectData.avatarUrl placeholderImage:[UIImage imageNamed:TUIKitResource(@"nsc_defautl")]];
    } else if (selectData.avatarImage) {
        [self.avatarView setImage:selectData.avatarImage];
    } else {
        [self.avatarView setImage:[UIImage imageNamed:TUIKitResource(@"nsc_defautl")]];
    }
