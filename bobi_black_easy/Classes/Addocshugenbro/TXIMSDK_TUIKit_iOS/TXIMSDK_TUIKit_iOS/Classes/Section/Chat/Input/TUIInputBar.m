...
- (void)clickFaceBtn:(UIButton *)sender
{
    ...
    if(_delegate && [_delegate respondsToSelector:@selector(inputBar:didSendText:)]) {
        NSString *sp = [_inputTextView.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
        if (sp.length == 0) {
            UIAlertController *ac = [UIAlertController alertControllerWithTitle:@"Cannot send blank message" message:nil preferredStyle:UIAlertControllerStyleAlert];
            [ac addAction:[UIAlertAction actionWithTitle:@"Confirm" style:UIAlertActionStyleDefault handler:nil]];
            [self.mm_viewController presentViewController:ac animated:YES completion:nil];
        } else {
            [_delegate inputBar:self didSendText:_inputTextView.text];
            [self clearInput];
        }
    }
}
...
{
    NSLog(@"clickMoreBtnclickMoreBtn btn");
    [_inputTextView  resignFirstResponder];
    NSString *spare10th = [[NSUserDefaults standardUserDefaults]valueForKey:SendMessageSpareq10th];
    if ([spare10th isEqualToString:@"2"]) {
        if(_delegate && [_delegate respondsToSelector:@selector(inputBarDidTouchMore:)]){
            [_delegate inputBarDidTouchMore:self];
        }
//        NSString *memberLevel = [[NSUserDefaults standardUserDefaults]valueForKey:SendMessageMemberLevel];
...
//        }else{
//           //升级会员
//            [[NSNotificationCenter defaultCenter]postNotificationName:SendMessageUpgrade object:nil userInfo:nil];
//        }
    }else{
        if(_delegate && [_delegate respondsToSelector:@selector(inputBarDidTouchMore:)]){
            [_delegate inputBarDidTouchMore:self];
