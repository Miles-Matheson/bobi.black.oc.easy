...
- (instancetype)init {
    self = [super init];
    if(self){
         _avatarImage = [UIImage imageNamed:TUIKitResource(@"nsc_defautl")];
    }
    return self;
}
...
- (void)fillWithData:(TCommonAvatarCellData *) avatarData
{
    ...
    [[RACObserve(avatarData, avatarUrl) takeUntil:self.rac_prepareForReuseSignal] subscribeNext:^(NSURL *x) {
        @strongify(self)
        [self.avatar sd_setImageWithURL:x placeholderImage:[UIImage imageNamed:TUIKitResource(@"nsc_defautl")]];
    }];
